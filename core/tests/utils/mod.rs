...
use stump_core::{
	db::{
		entity::{LibraryPattern, LibraryScanMode},
...
fn get_temp_library_root() -> TempDir {
	Builder::new()
...
impl TempLibrary {
	...
	pub fn collection_library() -> CoreResult<TempLibrary> {
		let root = get_temp_library_root();
	...
	pub async fn epub_library(
		...
		let temp_lib = TempLibrary::collection_library()?;

		let (library, options) =
			temp_lib.insert(client, LibraryScanMode::Default).await?;

		Ok((library, options, temp_lib))
	}
...
pub async fn persist_test_job(
	...
	let job = LibraryScanJob::new(library.id.clone(), library.path.clone());
	ctx.enqueue_job(job);
...
pub fn get_mock_config_file() -> String {
